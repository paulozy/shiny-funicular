...
import { backendApprovePullRequest } from '@/lib/api/pull_requests'
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    await backendApprovePullRequest(token, id, prNumber, typeof payload?.body === 'string' ? payload.body : undefined)
    ...
    log.error({ status: statusCode, error_code: backendCode }, 'approve_pr_route_error')
